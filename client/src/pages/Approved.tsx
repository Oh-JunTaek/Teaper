...
import { Download, FileDown, FileSpreadsheet, FileText, LockKeyhole, Printer, ShieldCheck, Sparkles } from "lucide-react";
...
export default function Approved() {
  ...
  const printWorkbook = async (kind: QuestionDocumentKind) => {
    if (!plan?.canUseWorkbookExport) return toast.error("문제집 출력 패키지는 교사 플러스 기능입니다.");
    setIsExporting(true);
    try {
      ...
      toast.error(error?.message || "문제집 출력 권한을 확인하지 못했습니다.");
    } finally {
      setIsExporting(false);
    }
  };
