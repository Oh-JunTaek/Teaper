...
/** 과거 생성값에 붙은 번호 접두어를 제거해 화면·인쇄에서 번호가 한 번만 보이게 한다. */
...
/** 기존 세트에는 문항 상태가 없으므로 모두 검수 대기로 보여 안전하게 다시 확인하게 한다. */
...
/** 세트 배지는 문항별 처리 현황을 요약할 뿐, 학생용 출력 권한은 각 문항의 승인 상태가 기준이다. */
...
function printApprovedQuiz(quiz: PrintableQuiz) {
  const safe = (value: string) => value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const content = quiz.questions.map((question, index) => `<article><p class="number">${index + 1}번 · ${safe(question.concept)}</p><h2>${safe(question.questionText)}</h2>${question.choices.length ? `<ol>${question.choices.map((choice, choiceIndex) => `<li>${safe(quizChoiceLabel(choice, choiceIndex))}</li>`).join("")}</ol>` : quiz.questionFormat === "short_answer" ? `<p class="answer-line">답: ________________________________________</p>` : ""}</article>`).join("");
...
/** 일반 문항 검수함과 분리된 간결한 쪽지시험. 한 세트 안에서도 문항별로 검수한다. */
export default function QuickQuiz() {
  ...
  const create = trpc.assessment.quickQuiz.create.useMutation({ onSuccess: result => { setSelectedQuizId(result.id); toast.success(`${result.questions.length}개 쪽지시험 문항을 만들었습니다.`); refresh(); }, onError: error => toast.error(error.message) });
