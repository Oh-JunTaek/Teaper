...
import { formatScienceNotation } from "@/lib/scienceNotation";
...
function quizChoiceLabel(choice: string, index: number) {
  ...
  return formatScienceNotation(`${choiceMarker(index)} ${cleaned || choice.trim()}`);
}
...
function quizAnswerLabel(answer: string, choices: string[]) {
  ...
  return formatScienceNotation(index >= 0 ? `${choiceMarker(index)}번` : answer.replace(/^선택\s*/i, ""));
}
...
function printApprovedQuiz(quiz: PrintableQuiz, includePoints: boolean, watermark: boolean) {
  const safe = (value: string) => formatScienceNotation(value).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
export default function QuickQuiz() {
  ...
  const [form, setForm] = useState(() => {
    const saved = window.localStorage.getItem("quick-quiz-last-level");
    const schoolLevel: "middle" | "high" = saved === "middle" ? "middle" : "high";
    return { schoolLevel, subject: schoolLevel === "middle" ? "중등 과학" : "화학 I", unit: schoolLevel === "middle" ? "화학 반응의 규칙과 에너지 변화" : "화학 결합", topic: "", difficulty: "보통" as "낮음" | "보통" | "높음", questionFormat: "multiple_choice" as keyof typeof formatLabel, questionCount: 3 };
  });
  const [providerId, setProviderId] = useState("managed");
  const [externalConsent, setExternalConsent] = useState(false);
  const [selectedQuizId, setSelectedQuizId] = useState<number | null>(() => {
    const setId = Number(new URLSearchParams(window.location.search).get("setId"));
  ...
  // 마지막 학교급을 이 브라우저에만 저장해 다음 쪽지시험의 기본 선택을 줄인다.
  useEffect(() => { window.localStorage.setItem("quick-quiz-last-level", form.schoolLevel); }, [form.schoolLevel]);
  ...
  // 문항별 검수 결과는 쪽지시험 목록·대시보드·승인 문항 보관함에서 같은 기준으로 즉시 다시 읽습니다.
  const refresh = () => void Promise.all([
    utils.assessment.quickQuiz.list.invalidate(),
    utils.assessment.dashboard.invalidate(),
    utils.assessment.questions.list.invalidate(),
  ]);
  ...
  // 승인 문항 보관함에서 특정 세트를 열면, 목록 갱신 뒤에도 해당 세트를 유지한다.
  useEffect(() => {
    const setId = Number(new URLSearchParams(location.split("?")[1] || "").get("setId"));
    if (Number.isInteger(setId) && setId > 0 && quizzes.data?.some(quiz => quiz.id === setId)) setSelectedQuizId(setId);
  }, [location, quizzes.data]);
  const saveQuestionPoints = (questionIndex: number, rawValue: string) => {
    if (!selectedQuiz) return;
    if (!rawValue.trim()) return updateQuestionPoints.mutate({ id: selectedQuiz.id, questionIndex, points: null });
  ...
  // 배점은 승인할 때 함께 저장한다. 빈 칸은 학생용 출력에서 배점을 보이지 않게 한다.
  const approveQuestion = (questionIndex: number, pointKey: string, rawValue: string) => {
    if (!selectedQuiz) return;
    if (!rawValue.trim()) return reviewQuestion.mutate({ id: selectedQuiz.id, questionIndex, status: "approved" });
    const points = Number(rawValue);
    if (!Number.isFinite(points) || points < 0 || points > 100 || Math.round(points * 10) !== points * 10) return toast.error("배점은 0~100점, 소수 첫째 자리까지 입력해 주세요.");
    setPointDrafts(current => ({ ...current, [pointKey]: String(points) }));
    reviewQuestion.mutate({ id: selectedQuiz.id, questionIndex, status: "approved", points });
  };
    ...
    <section className="mt-7 grid gap-5 lg:grid-cols-[410px_1fr]">
      <form onSubmit={submit} className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4">
          <div><Label>학교급</Label><select value={form.schoolLevel} onChange={event => { const schoolLevel = event.target.value as "middle" | "high"; setForm(current => ({ ...current, schoolLevel, subject: schoolLevel === "middle" ? "중등 과학" : "화학 I", unit: schoolLevel === "middle" ? "화학 반응의 규칙과 에너지 변화" : "화학 결합" })); }} className="mt-1.5 h-10 w-full rounded-md border border-input bg-white px-3 text-sm"><option value="middle">중등</option><option value="high">고등</option></select></div>
      ...
      <div>
        ...
        {quizzes.isLoading ? <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" />생성 결과를 불러오는 중입니다.</div> : selectedQuiz ? <div className="mt-4 space-y-4"><div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div><p className="font-semibold text-[#183248]">{formatScienceNotation(selectedQuiz.subject)} · {formatScienceNotation(selectedQuiz.unit)}</p><p className="mt-1 text-sm text-slate-500">{formatScienceNotation(selectedQuiz.topic)} · {formatLabel[selectedQuiz.questionFormat as keyof typeof formatLabel] ?? "객관식"} · {summary.text}</p><p className="mt-1 text-xs text-slate-400">생성: {quizCreatedAtLabel(selectedQuiz.createdAt)}</p></div><Badge className={summary.meta.className}>{summary.meta.label}</Badge></div>{approvedQuestions.length ? <div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={includePointsWhenPrinting} onChange={event => setIncludePointsWhenPrinting(event.target.checked)} />문제 배점 표기</label><span className="text-xs text-slate-500">{plan.data?.canRemoveStudentWatermark ? "교사 플러스: 워터마크 없음" : "교사 기본: 오른쪽 아래 여백에 워터마크 표시"}</span><Button size="sm" variant="outline" onClick={() => printApprovedQuiz({ subject: selectedQuiz.subject, unit: selectedQuiz.unit, topic: selectedQuiz.topic, questionFormat: (selectedQuiz.questionFormat as keyof typeof formatLabel) ?? "multiple_choice", questions: approvedQuestions }, includePointsWhenPrinting, !plan.data?.canRemoveStudentWatermark)}><Printer className="mr-1.5 h-4 w-4" />승인 문항 학생용 인쇄·PDF ({approvedQuestions.length})</Button></div> : null}{quizQuestions.map((question, index) => { const state = reviewStates[index]; const pointKey = `${selectedQuiz.id}-${index}`; const pointValue = pointDrafts[pointKey] ?? (typeof question.points === "number" ? String(question.points) : ""); return <article key={pointKey} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><Badge variant="secondary">{index + 1}번</Badge><Badge className="bg-[#F2FBF6] text-[#15856B] hover:bg-[#F2FBF6]">{formatScienceNotation(question.concept)}</Badge></div><Badge className={reviewMeta[state].className}>{reviewMeta[state].label}</Badge></div><p className="mt-4 font-semibold leading-7 text-[#183248]">{formatScienceNotation(question.questionText)}</p>{question.choices.length ? <ol className="mt-3 space-y-1 text-sm text-slate-600">{question.choices.map((choice, choiceIndex) => <li key={choiceIndex}>{quizChoiceLabel(choice, choiceIndex)}</li>)}</ol> : selectedQuiz.questionFormat === "short_answer" ? <p className="mt-3 text-sm text-slate-500">학생이 짧게 답을 작성하는 주관식 문항입니다.</p> : null}<div className="mt-4 rounded-xl bg-[#F7FCF9] p-3 text-sm leading-6 text-slate-700"><p><strong className="text-[#183248]">정답:</strong> {quizAnswerLabel(question.answer, question.choices)}</p><p className="mt-1"><strong className="text-[#183248]">해설:</strong> {formatScienceNotation(question.explanation)}</p></div><div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><Label htmlFor={`quick-points-${pointKey}`} className="text-sm font-semibold text-[#183248]">배점 부여</Label><div className="mt-2 flex flex-wrap items-center gap-2"><Input id={`quick-points-${pointKey}`} inputMode="decimal" type="number" min="0" max="100" step="0.1" value={pointValue} onChange={event => setPointDrafts(current => ({ ...current, [pointKey]: event.target.value }))} className="h-10 w-full min-w-36 flex-1 bg-white sm:max-w-48" placeholder="미입력 시 미표기" /><span className="text-sm text-slate-500">점</span>{state === "approved" ? <Button type="button" size="sm" variant="outline" disabled={updateQuestionPoints.isPending} onClick={() => saveQuestionPoints(index, pointValue)}>배점 변경 저장</Button> : null}</div>{state === "approved" ? <p className="mt-2 text-xs text-slate-500">승인한 문항의 배점은 이곳에서 다시 수정하거나 비워서 미표기로 바꿀 수 있습니다.</p> : null}</div><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" disabled={reviewQuestion.isPending || state === "approved"} onClick={() => approveQuestion(index, pointKey, pointValue)}><CheckCircle2 className="mr-1.5 h-4 w-4" />이 문항 승인</Button><Button size="sm" variant="outline" disabled={reviewQuestion.isPending || state === "revised"} onClick={() => reviewQuestion.mutate({ id: selectedQuiz.id, questionIndex: index, status: "revised" })}><PencilLine className="mr-1.5 h-4 w-4" />수정 필요</Button><Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" disabled={reviewQuestion.isPending || state === "rejected"} onClick={() => reviewQuestion.mutate({ id: selectedQuiz.id, questionIndex: index, status: "rejected" })}><XCircle className="mr-1.5 h-4 w-4" />이 문항 반려</Button></div></article>; })}</div> : <div className="mt-4 rounded-2xl border border-dashed border-[#B9DCCF] bg-[#F7FCF9] p-8 text-center"><FileText className="mx-auto h-7 w-7 text-[#15856B]" /><h2 className="mt-3 font-semibold text-[#183248]">아직 만든 쪽지시험이 없습니다.</h2><p className="mt-2 text-sm leading-6 text-slate-500">확인할 개념을 입력하면 현재 사용자 계정에만 저장되는 짧은 문제 세트가 준비됩니다.</p></div>}
      </div>
    </section>
  </div>;
}
