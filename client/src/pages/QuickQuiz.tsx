...
function printApprovedQuiz(quiz: { subject: string; unit: string; topic: string; questionFormat: keyof typeof formatLabel; questions: unknown[] }) {
  ...
  const content = questions.map((question, index) => `<article><p class="number">${index + 1}번 · ${safe(question.concept)}</p><h2>${safe(question.questionText)}</h2>${question.choices.length ? `<ol>${question.choices.map(choice => `<li>${safe(choice)}</li>`).join("")}</ol>` : quiz.questionFormat === "short_answer" ? `<p class="answer-line">답: ________________________________________</p>` : ""}</article>`).join("");
