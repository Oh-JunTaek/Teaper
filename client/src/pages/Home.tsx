...
import { ArrowRight, BookOpen, CalendarDays, ClipboardCheck, FileText, LoaderCircle, Sparkles, StickyNote, Timer } from "lucide-react";
...
const cards = [
  ...
  { key: "reviewCount", label: "검수 대기 문항", detail: "일반 문항·쪽지시험의 검수 대기", icon: ClipboardCheck, tone: "bg-[#FFF2D8] text-[#B56716]", path: "/review" },
  { key: "approvedCount", label: "승인된 문항", detail: "일반 문항·쪽지시험 보관·내보내기", icon: Sparkles, tone: "bg-[#F2EAFE] text-[#7B56B3]", path: "/approved" },
] as const;

const workCards = [
  { label: "간결한 쪽지시험", detail: "한 개념을 빠르게 확인하는 짧은 문항", icon: Timer, path: "/quick-quiz", tone: "bg-[#E8EFF7] text-[#2D6496]" },
  { label: "교사 메모장", detail: "AI에 자동 반영되지 않는 개인 작업 메모", icon: StickyNote, path: "/notes", tone: "bg-[#F2EAFE] text-[#7B56B3]" },
  { label: "시험일·일정", detail: "시험일·마감·검수 계획을 날짜별로 관리", icon: CalendarDays, path: "/schedule", tone: "bg-[#E6F4EE] text-[#15856B]" },
];

export default function Home() {
  const { data, isLoading } = trpc.assessment.dashboard.useQuery();
  const { data: schedules } = trpc.assessment.schedules.list.useQuery();
  ...
  const upcomingSchedules = schedules?.filter(item => item.status === "planned" && item.scheduleDate >= today).slice(0, 3) || [];
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-5 grid gap-4 md:grid-cols-2">{workCards.map(card => { const isQuickQuiz = card.path === "/quick-quiz"; const quickDetail = isQuickQuiz && !isLoading ? `검수 대기 ${data?.quickQuizReviewCount ?? 0}문항 · 승인 ${data?.quickQuizApprovedCount ?? 0}문항` : card.detail; return <button key={card.path} onClick={() => setLocation(card.path)} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#9CCFC0] hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#15856B]"><div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${card.tone}`}><card.icon className="h-5 w-5" /></div><div><h2 className="font-semibold text-[#183248]">{card.label}</h2><p className="mt-1 text-sm text-slate-500">{quickDetail}</p></div><ArrowRight className="ml-auto h-4 w-4 text-[#15856B]" /></button>; })}</section>
      <section className="mt-5 rounded-2xl border border-[#B9DCCF] bg-[#F7FCF9] p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-[#15856B]">접속 중 일정 안내</p><h2 className="mt-1 text-lg font-bold text-[#183248]">오늘과 다음 일정</h2><p className="mt-1 text-sm text-slate-500">이 안내는 현재 웹을 열어 둔 경우에만 표시됩니다. 브라우저 푸시나 외부 전송은 사용하지 않습니다.</p></div><Button variant="outline" onClick={() => setLocation("/schedule")} className="shrink-0 border-[#B9DCCF] bg-white text-[#15856B] hover:bg-white">일정 열기</Button></div>{upcomingSchedules.length ? <div className="mt-4 grid gap-2 md:grid-cols-3">{upcomingSchedules.map(item => <button key={item.id} type="button" onClick={() => setLocation("/schedule")} className="rounded-xl border border-[#D7EADF] bg-white p-3 text-left"><p className="truncate font-semibold text-[#183248]">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.scheduleDate}{item.scheduleTime ? ` · ${item.scheduleTime}` : ""}</p></button>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-[#B9DCCF] bg-white/70 p-3 text-sm text-slate-500">앞으로 예정된 일정이 없습니다. 시험일이나 검수 마감을 등록해 보세요.</p>}</section>
