...
object QuickQuizFormPolicy {
    ...
    fun difficultyTier(rate: Int): String = when { rate <= 30 -> "높음"; rate <= 60 -> "보통"; else -> "낮음" }
    fun difficultyLabel(rate: Int): String = "목표 정답률 $rate% · ${difficultyTier(rate)}${if (rate <= 30) " (조건 적용·간단 계산·오개념 판별)" else ""}"
