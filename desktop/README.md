...
| DOCX 시험지·정답 해설지 | 사용자가 선택한 로컬 경로 | 사용자가 직접 선택할 때만 |

모델 부재·실행 오류·성능 부족 시 이 기반은 외부 개인 API로 자동 전환하지 않는다. UI는 오류 원인과 함께 **수동 검수** 또는 사용자가 명시적으로 선택한 다른 로컬 모델 실행 방식을 제시해야 한다.

## 교사용 문서 내보내기

승인 문항은 CSV 외에 **시험지 DOCX**와 **정답·해설지 DOCX**로도 생성할 수 있다. 그래프·표가 있는 문항은 문서에 함께 재현한다. DOCX는 Word와 최신 한글에서 열어 편집할 수 있으며, 한글 전용 형식이 필요하면 한글에서 연 뒤 HWPX로 다시 저장한다. 초기 배포에서는 구형 HWP 직접 생성은 제공하지 않는다.

인쇄용 HTML 출력 계약도 준비되어 있어, 설치형 Electron 또는 Tauri 셸은 동일한 시험지·정답 해설 미리보기를 별도 창에 표시한 뒤 교사가 로컬 프린터 또는 ‘PDF로 저장’을 직접 선택하도록 연결한다. 실제 설치형 창·운영체제 PDF 저장 UI는 데스크톱 셸 단계에서 연결한다. 내보낸 시험지·정답 해설지는 민감한 시험 자료일 수 있으므로 공유 폴더·개인 클라우드 자동 동기화 위치가 아닌 학교 보안 지침에 맞는 경로에 보관해야 한다.
