...
import { readableMath } from "./generationResult.mjs";
...
export async function openLocalStore() {
  ...
  db.exec(`PRAGMA journal_mode = WAL;
    ...
    CREATE TABLE IF NOT EXISTS quick_quiz_sets (id TEXT PRIMARY KEY, subject TEXT NOT NULL, unit TEXT NOT NULL, topic TEXT NOT NULL, school_level TEXT NOT NULL DEFAULT 'high', difficulty TEXT NOT NULL, question_format TEXT NOT NULL DEFAULT 'multiple_choice', question_count INTEGER NOT NULL, raw_output TEXT NOT NULL, model TEXT NOT NULL, prompt_version TEXT NOT NULL, status TEXT NOT NULL, question_review_states TEXT, question_points TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
  ...
  // 기존 local-only 세트는 고등 기준으로 읽어 학교급 선택을 추가해도 검수·출력 기록을 잃지 않는다.
  try { db.exec("ALTER TABLE quick_quiz_sets ADD COLUMN school_level TEXT NOT NULL DEFAULT 'high'"); } catch (error) { if (!String(error?.message || error).includes("duplicate column name")) throw error; }
  ...
  return {
    ...
    saveQuickQuizSet(quiz) { const states = normalizeQuickQuizQuestionStates(quiz.questionReviewStates, quiz.questionCount); const points = normalizeQuickQuizQuestionPoints(quiz.questionPoints, quiz.questionCount); db.prepare("INSERT INTO quick_quiz_sets (id, subject, unit, topic, school_level, difficulty, question_format, question_count, raw_output, model, prompt_version, status, question_review_states, question_points, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(quiz.id, quiz.subject, quiz.unit, quiz.topic, quiz.schoolLevel === "middle" ? "middle" : "high", quiz.difficulty, quiz.questionFormat || "multiple_choice", quiz.questionCount, quiz.rawOutput, quiz.model, quiz.promptVersion, summarizeQuickQuizReview(states), JSON.stringify(states), JSON.stringify(points), quiz.createdAt, quiz.updatedAt); },
    listQuickQuizSets() { return db.prepare("SELECT *, COALESCE(question_format, 'multiple_choice') AS question_format, COALESCE(school_level, 'high') AS school_level FROM quick_quiz_sets ORDER BY updated_at DESC").all().map(quiz => { const states = normalizeQuickQuizQuestionStates(quiz.question_review_states, quiz.question_count); const points = normalizeQuickQuizQuestionPoints(quiz.question_points, quiz.question_count); return { ...quiz, question_review_states: JSON.stringify(states), question_points: JSON.stringify(points), questionReviewStates: states, questionPoints: points, status: summarizeQuickQuizReview(states) }; }); },
...
export function studentQuestionHeading(question, index, includePoints = false) {
  return `${index + 1}. ${readableMath(question.questionText)}${includePoints ? ` ［${question.points}점］` : ""}`;
}
...
export async function exportQuestionsDocx(questions, kind = "question-paper", options = {}) {
  ...
  questions.forEach((question, index) => {
    children.push(new Paragraph({ spacing: { before: index === 0 ? 0 : 280, after: 120 }, children: [new TextRun({ text: answerSheet ? `${index + 1}. ${readableMath(question.questionText)}` : studentQuestionHeading(question, index, options.includePoints === true), size: 22, bold: answerSheet })] }), ...(answerSheet ? [new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: `${question.questionType} · 난이도 ${question.difficulty} · ${question.points}점`, color: "475569", size: 18 })] })] : []));
    (question.choices || []).forEach((choice, choiceIndex) => children.push(new Paragraph({ indent: { left: 360 }, spacing: { after: 60 }, children: [new TextRun({ text: `${"①②③④⑤"[choiceIndex] || `${choiceIndex + 1}.`} ${readableMath(choice)}`, size: 21 })] })));
    children.push(...localVisualBlocks(question.visualSpec));
    if (answerSheet) children.push(new Paragraph({ spacing: { before: 120, after: 50 }, children: [new TextRun({ text: "정답  ", bold: true, color: "15856B" }), new TextRun({ text: readableMath(question.answer), bold: true })] }), new Paragraph({ spacing: { after: 50 }, children: [new TextRun({ text: "해설  ", bold: true, color: "183248" }), new TextRun(readableMath(question.explanation))] }), new Paragraph({ spacing: { after: 100 }, children: [new TextRun({ text: "출제 의도  ", bold: true, color: "183248" }), new TextRun(readableMath(question.intent))] }));
  });
  return Packer.toBuffer(new Document({ sections: [{ children }] }));
}

function escapeHtml(value) { return readableMath(String(value ?? "")).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#39;"); }
