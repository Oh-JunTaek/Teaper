...
function registerHandlers() {
  ...
  // 승인 시 적은 배점도 같은 local-only 요청에서 보관해 검수 결과와 학생용 출력 값이 어긋나지 않게 한다.
  ipcMain.handle("local:review-quick-quiz-question", (_event, input) => { const status = ["approved", "revised", "rejected"].includes(input.status) ? input.status : "revised"; if (status === "approved" && Object.hasOwn(input, "points")) { const points = input.points === null ? null : Number(input.points); if (points !== null && (!Number.isFinite(points) || points < 0 || points > 100 || Math.round(points * 10) !== points * 10)) throw new Error("배점은 0~100점, 소수 첫째 자리까지 입력해 주세요."); const saved = store.updateQuickQuizQuestionPoints({ id: String(input.id), questionIndex: Number(input.questionIndex), points, updatedAt: new Date().toISOString() }); if (!saved) throw new Error("배점을 정할 쪽지시험 문항을 찾을 수 없습니다."); } const updated = store.reviewQuickQuizQuestion({ id: String(input.id), questionIndex: Number(input.questionIndex), status, updatedAt: new Date().toISOString() }); if (!updated) throw new Error("검수할 쪽지시험 문항을 찾을 수 없습니다."); return { success: true, ...updated }; });
  // 승인 뒤에도 배점을 수정하거나 비워 학생용 출력에서 미표기로 되돌릴 수 있다.
  ipcMain.handle("local:update-quick-quiz-question-points", (_event, input = {}) => { const points = input.points === null ? null : Number(input.points); if (points !== null && (!Number.isFinite(points) || points < 0 || points > 100 || Math.round(points * 10) !== points * 10)) throw new Error("배점은 0~100점, 소수 첫째 자리까지 입력해 주세요."); const updated = store.updateQuickQuizQuestionPoints({ id: String(input.id), questionIndex: Number(input.questionIndex), points, updatedAt: new Date().toISOString() }); if (!updated) throw new Error("배점을 정할 쪽지시험 문항을 찾을 수 없습니다."); return { success: true, points: updated }; });
  ...
  ipcMain.handle("local:export-quick-quiz", async () => {
    const quizzes = store.listQuickQuizSets(); if (!quizzes.length) throw new Error("내보낼 쪽지시험이 없습니다.");
    const content = quizzes.map((quiz, index) => `# ${index + 1}. ${quiz.subject} · ${quiz.unit}\n개념: ${quiz.topic}\n난이도: ${quiz.difficulty}\n문항별 검수: ${(quiz.questionReviewStates || []).map((status, questionIndex) => `${questionIndex + 1}번 ${status}`).join(", ")}\n생성 모델: ${quiz.model}\n\n${quiz.raw_output}`).join("\n\n---\n\n");
