...
struct ApprovedItemBankView: View {
    ...
    var body: some View { NavigationStack { List { let regular = workspace.assessmentQuestions.filter { $0.reviewStatus == .approved && matches($0.title + " " + $0.content) }; let quick = workspace.approvedQuickQuizItems.filter { matches($0.question.questionText + " " + $0.set.topic) }
        ...
        if !regular.isEmpty { Section("일반 문항 \(regular.count)개") { ForEach(regular) { Text(ScienceNotation.format($0.title)) } } }
        if !quick.isEmpty { Section("승인 쪽지시험 문항 \(quick.count)개") { ForEach(quick) { item in VStack(alignment: .leading, spacing: 5) { Label("쪽지시험 · \(ScienceNotation.format(item.set.topic)) · \(item.questionIndex + 1)번", systemImage: "timer").font(.caption).foregroundStyle(.orange); Text(ScienceNotation.format(item.question.questionText)).font(.body); Text("\(item.set.schoolLevel) · \(ScienceNotation.format(item.set.subject)) · \(ScienceNotation.format(item.set.unit))").font(.caption).foregroundStyle(.secondary) } } }
