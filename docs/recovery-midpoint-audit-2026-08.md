...
| 쪽지시험 형식 | 객관식 4지선다·주관식·O/X 선택, 형식별 서버 정규화, 승인 뒤 학생용 인쇄/PDF | 같은 형식 저장·생성 지침, SQLite·암호화 백업 포함, 승인 뒤 학생용/교사용 TXT | 같은 형식 저장·생성 지침, 앱 전용 저장소, 승인 뒤 학생용 텍스트 공유 |
| 숫자형 객관식 보기 | 선택 번호와 값은 `선택 ①: 4`, 정답은 `선택 ①`로 구분 | 새 생성·기존 화면·학생용 TXT를 같은 표기로 보정 | 새 생성·기존 화면·학생용 텍스트 공유를 같은 표기로 보정 |
...
| P1 | 숫자형 보기 실환경 검증 | 숫자 값이 포함된 객관식에서 `선택 ①: 값` 표기와 `정답: 선택 ①`가 웹·Windows·S25+의 검수·학생용 출력에서 일관되게 보이는지 확인해야 함 |
