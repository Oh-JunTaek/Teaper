...
| 쪽지시험 | 객관식 4지선다·주관식·O/X 선택, 독립 검수, 승인 뒤 학생용 인쇄/PDF 저장 | 같은 3가지 형식의 local-only 생성·저장·학생용/교사용 TXT 내보내기 | 같은 3가지 형식의 온디바이스 생성·검수·학생용 텍스트 공유 |
...
| 6 | **간결한 쪽지시험**에서 한 개념을 빠르게 확인할 세트를 만듭니다. | `객관식 (4지선다)`, `주관식`, `O/X` 중 하나를 고릅니다. 객관식 보기는 **`선택 ①: 4`**처럼 선택 번호와 숫자 값을 분리해 표시합니다. 쪽지시험은 일반 문항 검수함과 별도로 `승인`, `수정 필요`, `반려` 상태를 기록합니다. 승인 뒤 학생용 출력에는 정답·해설을 넣지 않습니다. |
...
웹은 TypeScript 검사와 Vitest **81개**를 통과했습니다. Windows beta.9은 local-only 저장소·보안·대화·생성 결과 조판·일정 저장·기기 알림 예약·쪽지시험 형식 저장 테스트와 JavaScript 구문 검사를 통과했습니다. Android 일정·알림·쪽지시험 형식 코드는 앱 전용 저장소·앱 잠금·재부팅 뒤 재예약 경로에 추가했으나, 복구된 샌드박스에는 Android SDK가 없어 이번 Gradle 단위 테스트를 시작하지 못했습니다. 실제 운영 전에는 S25+에서 일정 저장·수정·삭제·알림 허용/거부·재부팅 뒤 알림·앱 잠금 재진입을 포함한 쪽지시험·이미지 대화·오프라인·문서 공유를, Windows에서 설치형 앱·AI 도움 기능 준비·연속 대화·앱 실행 중 일정 알림·일정 백업·문서 출력을 별도로 확인해야 합니다.
