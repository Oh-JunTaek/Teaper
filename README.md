...
| iOS 준비 기반 | 해당 없음 | 해당 없음 | SwiftUI·LiteRT-LM 기반의 local-only 쪽지시험·승인 문항·대시보드·학생 공유 구조. Xcode·iPhone 검증 뒤 TestFlight·App Store 심사를 진행 |
...
웹은 TypeScript 검사와 Vitest **87개**를 통과했습니다. Windows beta.9은 local-only 저장소·보안·대화·생성 결과 조판·일정 저장·기기 알림 예약·**문항별 쪽지시험 검수·배점 저장·승인 문항 필터·대시보드 합산·백업 복구** 코드와 JavaScript 구문·Node 테스트를 확인했습니다. Android 일정·알림·문항별 쪽지시험 검수·배점·대시보드 표시는 앱 전용 저장소·앱 잠금·재부팅 뒤 재예약 경로에 추가했으나, 복구된 샌드박스에는 Android SDK가 없어 이번 Gradle 단위 테스트를 시작하지 못했습니다. iOS에는 SwiftUI·LiteRT-LM 기반과 XCTest 초안을 추가했지만 Linux 환경에는 Xcode·iPhone Simulator가 없어 컴파일하지 못했습니다. 실제 운영 전에는 S25+에서 문항별 승인·수정 필요·반려, `0·0.1·2·3·4·100점` 배점, 배점 표시/미표시 PDF, 부분 승인 세트의 학생용 공유, **대시보드의 일반/쪽지시험 합산**, 승인 문항 보관함의 일반/쪽지시험 구분을 확인하고, Windows에서 설치형 앱·부분 승인 학생용 PDF/TXT·대시보드 합산·승인 문항 보관함을 별도로 확인해야 합니다. iOS는 iPhone에서 모델 파일 가져오기·E2B/E4B 준비·문항별 검수·배점 표시 PDF·승인 문항 공유를 거쳐 TestFlight 검증 후에만 App Store 제출을 검토합니다.
