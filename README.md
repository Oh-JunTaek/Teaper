...
| 쪽지시험 | 객관식·주관식·O/X 선택, **문항별** 검수·교사 배점·승인 문항 학생용 PDF 저장 | 같은 3가지 형식의 local-only 생성·문항별 검수·배점·학생용 PDF/교사용 TXT 내보내기 | 같은 3가지 형식의 온디바이스 생성·문항별 검수·배점·승인 문항 PDF/텍스트 공유 |
...
| iOS 준비 기반 | 해당 없음 | 해당 없음 | SwiftUI·LiteRT-LM 기반의 local-only **쪽지시험·승인 문항·대시보드·학생 공유 준비 구조**. 자료·메모·일정·일반 문항·AI 채팅은 아직 미구현이며, Xcode·iPhone 검증과 기능 보강 뒤 TestFlight·App Store 심사를 진행 |
...
| 6 | **간결한 쪽지시험**에서 한 개념을 빠르게 확인할 세트를 만듭니다. | 중등·고등, `객관식`·`주관식`·`O/X`, 낮음·보통·높음 중 하나를 고릅니다. 객관식 보기는 **`① 내용`**, 정답은 **`①번`**처럼 짧고 분명하게 표시합니다. 세트 안의 **각 문항**을 `승인`, `수정 필요`, `반려`로 따로 검수합니다. 교사는 정답·해설을 확인한 뒤 `배점 부여`에 `0~100점` 범위의 소수 첫째 자리 값을 입력하고 문항을 승인합니다. 빈 값은 학생용에 표시하지 않으며, 승인 뒤에도 보관함에서 수정할 수 있습니다. 학생용 문서·PDF·텍스트를 만들 때 `문제 배점 표기`를 고르면 배점은 문항 끝의 **`［n점］`**으로만 출력합니다. 난이도·문항 유형·생성 정보와 정답·해설·개념 메모는 학생용 출력에 넣지 않습니다. |
...
| 학생용 PDF·인쇄의 EunmaStudio 표기 | 오른쪽 아래 여백에 표시 | 표시하지 않음 |
...
웹은 TypeScript 검사, Vitest **31파일·99테스트**, 프로덕션 빌드, 운영 의존성 감사 high 이상 0건을 통과했습니다. 일반 문항 생성은 복잡한 구조화 응답이 비어도 간결한 문항 형식으로 한 번 더 요청하도록 보완했으며, 비민감 예제로 생성과 독립 검증이 모두 완료되는 것을 확인했습니다. Windows beta.9은 local-only 저장소·보안·대화·생성 결과 조판·일정 저장·기기 알림 예약·**문항별 쪽지시험 검수·배점 저장·승인 문항 필터·대시보드 합산·백업 복구·학생용 워터마크** 코드와 JavaScript 구문·Node 테스트 5건·운영 의존성 감사 high 이상 0건을 확인했습니다. 학생용 DOCX·PDF·텍스트는 문제·보기와 교사가 선택한 배점만 포함하며, 난이도·문항 유형·검수·생성 정보·정답·해설·개념 메모를 포함하지 않습니다. 화학식·이온식·전자배치의 `H₂O`·`SO₄²⁻`·`1s²` 표기는 웹·Windows·Android·iOS 화면과 학생용 파일을 만들기 직전에 공통 정규화합니다. 쪽지시험은 중등·고등과 낮음·보통·높음 중 하나를 선택할 수 있습니다. Android 일정·알림·문항별 쪽지시험 검수·배점·대시보드·학생용 워터마크 표시는 앱 전용 저장소·앱 잠금·재부팅 뒤 재예약 경로에 추가했으나, 복구된 샌드박스에는 Android SDK가 없어 이번 Gradle 단위 테스트를 시작하지 못했습니다. iOS에는 SwiftUI·LiteRT-LM 기반과 XCTest 초안, 기본 플랜 학생용 PDF 워터마크 정책을 추가했지만 Linux 환경에는 Xcode·iPhone Simulator가 없어 컴파일하지 못했습니다. 실제 운영 전에는 S25+에서 문항별 승인·수정 필요·반려, `0·0.1·100점` 배점, 배점 표시/미표시 PDF, 기본·플러스 배포본의 워터마크 위치·미표시 대상을 확인하고, Windows에서 설치형 앱·학생용 PDF/TXT·기본·플러스 출력 구분을 별도로 확인해야 합니다. iOS는 iPhone에서 모델 파일 가져오기·E2B/E4B 준비·문항별 검수·배점 표시 PDF·기본·플러스 워터마크·승인 문항 공유를 거쳐 TestFlight 검증 후에만 App Store 제출을 검토합니다.

> 실제 출제 예정 문항, 학생 식별 정보, 민감한 성적 자료를 파일럿 테스트 이미지나 외부 개인 AI 요청에 사용하지 마세요.

## 출시 준비 판정

2026-08-27 기준으로 **정식 공개 출시는 보류**입니다. 웹은 공개 정책·지원 URL, 초대 파일럿 통제, 엣지 속도 제한·경보, 법무 검토를 마친 뒤에만 제한 파일럿을 검토할 수 있습니다. Windows와 Android는 실제 설치·기기 검증 전의 내부/제한 테스트 단계이고, iOS는 TestFlight 이전의 준비 기반입니다. 이번 코드·의존성·저장소 접근 보완과 플랫폼별 차단 요건은 [출시 준비 점검 보고서](./docs/release-readiness-audit-2026-08.md)에, 공식 기준 링크는 [출시 공식 출처 메모](./docs/release-source-notes-2026-08.md)에 정리했습니다.
