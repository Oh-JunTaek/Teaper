...
웹은 TypeScript 검사와 Vitest **77개**를 통과했습니다. Windows beta.9은 local-only 저장소·보안·대화·생성 결과 조판 테스트와 JavaScript 구문 검사를 통과했고, Android는 단위 테스트·Debug APK 조립을 통과했습니다. 다만 실제 운영 전에는 S25+에서 쪽지시험·이미지 대화·오프라인·앱 잠금·문서 공유를, Windows에서 설치형 앱·AI 도움 기능 준비·연속 대화·문서 출력을 별도로 확인해야 합니다.
