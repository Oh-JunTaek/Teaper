...
| AI 채팅 | 준비 중 | 최근 대화·모델 예열·제목/즐겨찾기·이 PC 저장 | Gemma 기반 현장형 대화·공유·이미지 질문 |
...
Windows 앱은 교사에게 모델 실행 방식이나 개발 용어를 요구하지 않습니다. 처음에는 **AI 설정 → AI 도움 기능 준비**에서 안내된 기본 도움 기능을 한 번 준비한 뒤, 문항 생성 또는 AI 채팅에서 바로 선택하면 됩니다. 선택 목록에는 **이 PC에 준비된 도움 기능만** 표시되며, 마지막 선택은 다음 채팅에도 유지됩니다.

AI 채팅을 열면 선택한 도움 기능을 미리 준비합니다. 따라서 첫 준비 중에도 질문을 작성할 수 있고, 화면에는 `모델 로딩 중 → 질문 분석 중 → 답변 생성 중`의 순서로 현재 단계를 안내합니다. 대화는 최근 맥락을 이어가며, 제목 변경·즐겨찾기·삭제는 모두 이 PC의 대화 기록 안에서 처리됩니다.
...
웹은 TypeScript 검사와 Vitest **77개**를 통과했습니다. Windows local-only 저장소·보안·대화·생성 결과 조판 테스트와 Android 단위 테스트·Debug APK 조립도 통과했습니다. 다만 실제 운영 전에는 S25+에서 쪽지시험·이미지 대화·오프라인·앱 잠금·문서 공유를, Windows에서 설치형 앱·AI 도움 기능 준비·연속 대화·문서 출력을 별도로 확인해야 합니다.
