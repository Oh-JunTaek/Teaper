...
웹은 TypeScript 검사와 Vitest **86개**를 통과했습니다. Windows beta.9은 local-only 저장소·보안·대화·생성 결과 조판·일정 저장·기기 알림 예약·**문항별 쪽지시험 검수·승인 문항 필터·대시보드 합산·백업 복구** 코드와 JavaScript 구문·Node 테스트를 확인했습니다. Android 일정·알림·문항별 쪽지시험 검수·대시보드 표시는 앱 전용 저장소·앱 잠금·재부팅 뒤 재예약 경로에 추가했으나, 복구된 샌드박스에는 Android SDK가 없어 이번 Gradle 단위 테스트를 시작하지 못했습니다. iOS에는 SwiftUI·LiteRT-LM 기반과 XCTest 초안을 추가했지만 Linux 환경에는 Xcode·iPhone Simulator가 없어 컴파일하지 못했습니다. 실제 운영 전에는 S25+에서 문항별 승인·수정 필요·반려, 부분 승인 세트의 학생용 공유, **대시보드의 일반/쪽지시험 합산**, 승인 문항 보관함의 일반/쪽지시험 구분, 검수 대기 수와 최근 생성 시각 표시를 확인하고, Windows에서 설치형 앱·부분 승인 학생용 TXT·대시보드 합산·승인 문항 보관함을 별도로 확인해야 합니다. iOS는 iPhone에서 모델 파일 가져오기·E2B/E4B 준비·문항별 검수·승인 문항 공유를 거쳐 TestFlight 검증 후에만 App Store 제출을 검토합니다.
