...
| 6 | **간결한 쪽지시험**에서 한 개념을 빠르게 확인할 세트를 만듭니다. | `객관식`, `주관식`, `O/X` 중 하나를 고릅니다. 객관식 보기는 **`① 내용`**, 정답은 **`①번`**처럼 짧고 분명하게 표시합니다. 세트 안의 **각 문항**을 `승인`, `수정 필요`, `반려`로 따로 검수하고 교사가 `0~100점` 범위에서 소수 첫째 자리까지 배점을 확정합니다. 2·3·4점은 빠른 선택으로 정할 수 있습니다. 학생용 PDF를 만들 때는 배점 표시 여부만 선택하며, 표시한 배점은 문항 끝의 **`［n점］`**으로만 출력합니다. 난이도·문항 유형·생성 정보와 정답·해설·개념 메모는 학생용 출력에 넣지 않습니다. |
