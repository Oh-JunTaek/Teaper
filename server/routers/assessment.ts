  ...
  generation: router({
    ...
    create: protectedProcedure.input(z.object({
      ...
      for (let index = 0; index < input.questionCount; index += 1) {
        ...
        const sources = [
          ...rankedChunks.map(({ item }) => ({ sourceType: item.material.materialType === "guideline" ? "guideline" as const : "material" as const, sourceId: item.material.id, excerpt: item.chunk.content.slice(0, 500), sourceSnapshot: { title: item.material.title, fileName: item.material.fileName, fileUrl: item.material.fileUrl, mimeType: item.material.mimeType, sourceLocation: item.material.sourceLocation || `검색 발췌 ${item.chunk.chunkIndex + 1}번`, chunkIndex: item.chunk.chunkIndex, materialType: item.material.materialType } })),
          ...rankedReferences.map(({ item }) => ({ sourceType: "reference_question" as const, sourceId: item.id, excerpt: item.intent, sourceSnapshot: { source: item.source, year: item.year, questionNumber: item.questionNumber || null, sourceLocation: item.sourceLocation || null, sourceFileName: item.sourceFileName || null, sourceFileUrl: item.sourceFileUrl || null, questionType: item.questionType, intent: item.intent } })),
        ];
